...
lambdas = np.logspace(-3, 7, 11)                    # penalties
...
for i, deg in enumerate(N_pol):
    for j, lambda_ in enumerate(lambdas):
        ...
        # fit
        beta, beta_var = Ridge(X_train, z_train, lambda_, r_var=1)
...
plt.title("Ridge prediction errors", fontsize=15)
cb = plt.colorbar()
cb.set_label(label="Squared error", fontsize=12)
plt.savefig("Figures/ridge_grid_search.png", dpi=300)
plt.show()
